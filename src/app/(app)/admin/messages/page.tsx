  ...
  return (
    <div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-2">
        <section>
          <h2 className="mb-3 text-lg font-bold text-foreground">Sent messages</h2>
          {messages.length === 0 ? (
                ...
                <Card key={m.id} className="py-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-foreground">{m.subject}</p>
                  ...
                  <p className="mt-1.5 line-clamp-2 text-xs text-muted-foreground">{m.body}</p>
                </Card>
              ))}
            </div>
          )}
        ...
        <section>
          <h2 className="mb-3 text-lg font-bold text-foreground">Outbox (every email)</h2>
          <Card className="divide-y divide-line p-0">
            {recentEmails.map((e) => (
              <Link
                key={e.id}
                href={`/admin/messages/email/${e.id}`}
                className="flex items-center justify-between gap-3 px-4 py-2.5 hover:bg-background"
              >
                <div className="min-w-0">
            ...
            {recentEmails.length === 0 ? (
              <p className="px-4 py-6 text-center text-sm text-muted-foreground">Nothing sent yet.</p>
            ) : null}
          </Card>
        </section>
      </div>
